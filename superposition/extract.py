...
fr = open('test/lcb_results_2.txt','r')        
file_download = open('download_2.sh','w')
pdb_ids = []
file_extract = open('extract_2.sh','w')
line = fr.readline()
line_count = 0
pdb_files_count = 0
file_simulate = open('simulate_2.sh','w')
while line != '':
  x = line.strip('\n')
  y = x.split()
  pdb_id = y[0]
  ...
  line_count += 1
  simulate_cmd = './superimpose --proteins test/query.pdb ' + extracted_file_path + ' '
  simulate_cmd += '--accept 0.001 --translate 0.001 --rotate 0.05 --estimate general'
